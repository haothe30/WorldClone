...
namespace ScratchCardAsset
{
    public class ScratchCard : MonoBehaviour
    {
        ...
        public GameObject GetchildEarse()
        {
            return childEarse;
        }    
        ...
        [SerializeField] private ScratchMode _mode = ScratchMode.Erase;
        ...
        {
            get
            { return surfaceSpriteRenderer; }
            set
            { surfaceSpriteRenderer = value; }
        }

        public void SetUpEarase(GameObject earase)
        {
        ...
        public void OnStart()
        {
            ...
            ;
            // Debug.LogError("chiled Earse:" + childEarse.gameObject.name);
            Init();
        }
        void OnDestroy()
        {
        ...
        private void GetScratchBounds()
        {
            ...
            {
                //   Debug.LogError("================= loi~" + gameObject.transform.parent.transform.parent.name);
        ...
        public void ChangeColorSpriteRender(float targetAlpha, bool fix)
        {
            // Kiểm tra null
            if (surfaceSpriteRenderer == null)
            {
                Debug.LogError("surfaceSpriteRenderer is null!");
                return;
            }
            ...
            if (!fix)
            {
                ...
                // Lấy alpha hiện tại
                float currentAlpha = surfaceSpriteRenderer.color.a;

                // Nếu alpha hiện tại <= 0.5, không thực hiện
                if (currentAlpha <= 0.5f)
                {
                  //  Debug.Log("Alpha đã đạt ngưỡng 0.5, không thực hiện thêm tween.");
                    return;
                }

                // Đảm bảo targetAlpha không nhỏ hơn 0.5
                targetAlpha = Mathf.Max(targetAlpha, 0.2f);

                // Tính thời gian tween tỷ lệ nghịch với targetAlpha
                float maxDuration = 2f; // Thời gian tối đa
                float duration = maxDuration / targetAlpha;

                // Thực hiện tween alpha
                Color currentColor = surfaceSpriteRenderer.color;
                surfaceSpriteRenderer.DOColor(new Color(currentColor.r, currentColor.g, currentColor.b, targetAlpha), duration)
                    .OnUpdate(() =>
                    {
            // Dừng tween nếu alpha <= 0.5
            if (surfaceSpriteRenderer.color.a <= 0.2f)
                        {
                          //  Debug.Log("Alpha đã giảm đến hoặc dưới 0.5, dừng tween.");
                            surfaceSpriteRenderer.DOKill();
                        }
                    });
            }
            else
            {
                ...
                surfaceSpriteRenderer.color = currentColor;
            }
        }

        private void OnScratchStart()
        {
            //if (GameController.gameController.CheckCanXoa())
            //    return;
            cardRenderer.IsScratched = false;
