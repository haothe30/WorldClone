...
namespace ScratchCardAsset.Core
{
	...
	private bool isWebgl = true;
		...
		Vector2 PosTemp()
        {
			return Camera.main.WorldToScreenPoint(scratchCard.GetchildEarse().transform.position);

		}
		public void Update()
		{
			if (scratchCard.GetchildEarse() == null)
				return;
            ...
            if (Input.touchSupported && Input.touchCount > 0 && !isWebgl)
			{
				foreach (var touch in Input.touches)
				{
					...
					{
						TryScratch(fingerId, /*touch.position*/PosTemp());
					}
			...
			else
			{
				if (Input.GetMouseButtonDown(0))
				{
                  ...
                  //  Debug.LogError("zoooooooooo 1");
                }
				if (Input.GetMouseButton(0))
				{
					TryScratch(0, /*Input.mousePosition*/PosTemp());
                //    Debug.LogError("zoooooooooo 2");
                }
				if (Input.GetMouseButtonUp(0))
				{
					isScratching[0] = false;
                 //   Debug.LogError("zoooooooooo 3");
                }
			}
		}
		...
		private void TryScratch(int fingerId, Vector2 position)
		{
         ...
         //      Debug.LogError("pos:" + position + ":" + scratchCard.posChileEarse());
			}
